...
(this will terminate the print)."))
    ...
    def do_test_func(self, text):
        import shlex
        args = shlex.split(text)  # Assuming 'l' contains space-separated arguments
        if len(args) < 2:
            self.logError("Not enough arguments provided.")
            return
        text = args[0]
        tmp = args[1]
        gcode_script_path = '/home/epics/Printrun/text-to-gcode.py'
        gcode_output_path = 'output.gcode'
        script_args = [
            gcode_script_path,
            '-o', gcode_output_path,
        ...
        try:
            ...
            self.logError("Failed to generate GCode: " + str(e))
            return

        # Step 2: Load GCode into pronsole
        self.do_load(gcode_output_path)

        # Step 3: Print the GCode
        if self.fgcode:  # Check if GCode was successfully loaded
            self.do_print("")
